...
function submitComment() {
    ...
    //Bu satır, alınan verilerin boş olup olmadığını kontrol eder. Bos degilse yorum eklemeye devam eder.
    if (commentText.trim() !== "" && commenterName.trim() !== "") {
        //yeni bir div öğesi oluşturur ve bu öğenin içeriğini belirler. Strong arasindakiler kalin gozukur.
        var newComment = document.createElement('div');
        newComment.innerHTML = '<strong>' + commenterName + ':</strong> ' + commentText;

        // yeni yorumu .comments sınıfına sahip başka HTML öğesine ekle.
        document.querySelector('.comments').appendChild(newComment);

        // Formu temizle
        document.getElementById('commentForm').reset();
    } else {
        alert("Lütfen boş alanlari doldurun.");
    }
}
